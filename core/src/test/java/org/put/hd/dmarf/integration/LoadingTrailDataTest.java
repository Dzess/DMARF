 ...
 * Acceptance level of test for data loading. Using 
 * the default {@link IDataReprsentatinoBuilder}
 * @author Piotr
 * 
 */
public class LoadingTrailDataTest {
	...
	public void providing_trial_data_results_in_proper_data_representation() {
		...
		// perform test
		String pathToFile= "resources" + File.separator +
				"data" + File.separator +
				"trail.dat";
		...
		Assert.assertEquals(threeAttribute,result.getAttributesCounter().get(3));
		...
		Assert.assertEquals(numberOfTransactions, result.getTransactionsList().size());
		Assert.assertEquals(numberOfAttributes, result.getAttributesCounter().keySet().size());
		
		// verify the structure of the results presented
		// TODO: write this verification
	}
	
	// TODO: maybe writing more tests for special stupid cases on the integration level
}
